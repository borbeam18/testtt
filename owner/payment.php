...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'approve') {
    ...
    header('Location: /testtt/owner/payment.php?filter=' . $filter);
    exit;
}
...
$stats = [
  ...
  'approved' => $pdo->query("SELECT COALESCE(SUM(amount),0) FROM Payment WHERE verify_status='อนุมัติแล้ว' AND strftime('%m', payment_date)=strftime('%m', 'now') AND strftime('%Y', payment_date)=strftime('%Y', 'now')")->fetchColumn(),
