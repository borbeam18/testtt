...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header('Location: /testtt/owner/expenses.php');
    exit;
}

$stats = [];
$stats['income'] = $pdo->query("SELECT COALESCE(SUM(amount),0) FROM Payment WHERE verify_status='อนุมัติแล้ว' AND strftime('%m', payment_date)=strftime('%m', 'now') AND strftime('%Y', payment_date)=strftime('%Y', 'now')")->fetchColumn();
$expenseStats = $pdo->prepare("SELECT COALESCE(SUM(amount),0) FROM Expense WHERE owner_id = ? AND strftime('%m', expense_date) = strftime('%m', 'now') AND strftime('%Y', expense_date) = strftime('%Y', 'now')");
